...
To show an icon in the cells of a column `getCellIcon` function should be defined. The function takes a cell data as an argument and should return a string with the icon name (or `false` if no icon should be shown).

The icon name should be one of [the Vanilla built in icons](https://vanillaframework.io/docs/patterns/icons) such as "plus", "minus", "success", "error", etc.
...
columns = {
  ...
  accessor: "status",
  getCellIcon: ({ value }) => {
    return value === "success" ? "success" : false;
  }
}
```
